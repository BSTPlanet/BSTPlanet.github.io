...
export class RB extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('background_grayRed', 'Assets/background_planet_grayRed_singleLarge.png');
        this.load.image('onion', 'Assets/alien_pink.png'); // pink player-alien
    ...
    create() {
        ...
        this.add.image(10_000,750,'background_grayRed').setDepth(-1);
        // background = this.add.tileSprite(10_000,300,20_000,20_000,'background_tile').setDepth(-1);
        // background.tilePositionX = 15_000;

        // Text on top of the game world
        var text1 = this.add.text(9_000,100, 'Red Black BST', { fontFamily: 'nasalization-rg', fontSize: '30px', fill: '#000' });
        //Instructions
        ...
        keyR.on('down', () => {
            destroyEverything();
            this.scene.restart('RB');
            this.input.keyboard.removeAllKeys(true);
        });

        // *************PLAYER*************
        player = this.physics.add.sprite(10_000, 300, 'onion');
        ...
        // OLD EXPANSION:
        // nodes merge:
        // var numsToInsert = [963, 331, 427, 656, 419, 223, 754, 438, 104, 424, 24, 470, 818, 402, 687, 69, 560, 514, 876, 624, 449, 606, 944, 155, 814, 32, 673, 391, 620, 7];
        // crossing:
        // var numsToInsert = [295, 853, 519, 415, 289, 22, 885, 134, 972, 25, 712, 21, 278, 748, 866, 635, 371, 343, 389, 614, 35, 888, 339, 701, 566, 861, 659, 782, 461, 630];
        // %%%%%%%%%%%%%%%%%%%%%%
        ...
        // tree.redraw(this);
        // console.log(tree.root.right.distanceFromParent);
       ...
       // *************** TASKS + TASK ACTIONS ***************
        ...
        function displayTask(scene) {
            if (tasks.length != 0) { 
                taskText.setText('Inserting ' + tasks[0]);
            } else {
            ...
            if (nodeToDelete == null) {
                ...
                // if (node.key != 'null' && (tasks[0] == node.key || (tasks[0] == 'Min' && node.key == min(tree.root)) || (tasks[0] == 'Max' && node.key == max(tree.root))) && nodeToDelete == null) {
                    if (node.left.key =='null' && node.right.key =='null') { //  both children are null 
                        ...
                        this.add.tween({
                            targets: [node.left.link, node.right.link, node.left, node.right, node, node.keyString, node.left.keyString,node.right.keyString],
                            ease: 'Sine.easeOut',
                            duration: 1000,
                            alpha: '-=1'
                        });
                        ...
                        if (node.right.key == 'null') { // right child is null
                            ...
                            this.add.tween({
                                targets: [node.right, node, node.keyString, node.right.keyString],
                                ease: 'Sine.easeOut',
                                duration: 1000,
                                alpha: '-=1'
                            });
                            ...
                            this.add.tween({
                                targets: [node.left, node, node.keyString, node.left.keyString],
                                ease: 'Sine.easeOut',
                                duration: 1000,
                                alpha: '-=1'
                            });
        ...
        function checking(node) {
            if (node != null) {
                ...
                if (node.key == 88) {
                    console.log("88s depth: " + node.dpth);
                }
                checking(node.left);
                checking(node.right);
            }
            ...
            if(nodeToDelete != null && node.key != 'null'){
                ...
                if(node.key == key){
                    ...
                    if (nodeToDelete.right.left.key == 'null') { // when nodeToDelete's right child IS min (move min and its right subtree up)
                        ...
                        this.add.tween({
                            targets: [nodeToDelete, nodeToDelete.curtain, nodeToDelete.keyString, node.left, node.left.keyString],
                            delay: 1000,
                            ease: 'Sine.easeOut',
                            duration: 1000,
                            alpha: '-=1'
                        ...
                        if (node.right.key == 'null') {
                            ...
                            this.add.tween({
                                targets: [nodeToDelete, nodeToDelete.curtain, nodeToDelete.keyString, node.left, node.left.keyString, node.right, node.right.keyString],
                                delay: 1000,
                                ease: 'Sine.easeOut',
                                duration: 1000,
                                alpha: '-=1'
                            ...
                            this.time.addEvent({
                                ...
                                callback: function(nodeToUseForAppear,nodeToDelete,node,scene) {
                                    ...
                                    scene.add.tween({
                                        targets: [nodeToUseForAppear.left, nodeToUseForAppear.left.keyString],
                                        ease: 'Expo.easeIn',
                                        duration: 1000,
                                        alpha: '+=1'
                                    });
        ...
        function insert(player,node) {
            if(node.key == 'null') {
                ...
                if (tree.isRB) {
                    ...
                    node.isRed = true;
                    node.changeLinkColour(this);

                    // create left child
                    var childL = new NodeBST(this, node.posX-tree.w, node.posY+tree.z, 'null',node.dpth+1,node,true);
                    childL.distanceFromParent = -tree.w;
                    tree.nodearray.push(childL);

                    // create right child
                    var childR = new NodeBST(this, node.posX+tree.w, node.posY+tree.z, 'null',node.dpth+1,node,true);
                ...
                this.add.tween({
                    targets: [node, node.keyString, node.curtain, childL, childL.keyString, childL.link, childR, childR.keyString, childR.link],
                    ease: 'Sine.easeIn',
                    duration: 1000,
                    alpha: "+=1"
                });
        ...
        function check() { // pass root 
            var perfect = false;
            while (perfect == false) {
                // console.log("checking in while");
                // console.log("we pass to checkRBTreeLinks in loop this node: " + tree.root.key);
                // console.log("Is root's left Red?: " + tree.root.left.isRed);
                // console.log("Is root's right Red?: " + tree.root.right.isRed);
                checkRBTreeLinks(tree.root);
        ...
        function checkRBTreeLinks(node) { // pass root
            if (node != null) {
                everythingIsCorrect = checkRBLinks(node) && everythingIsCorrect;
                // console.log("doing recursion");
                checkRBTreeLinks(node.left);
                checkRBTreeLinks(node.right);
            }
        }
        ...
        function checkRBLinks(node) { // checkRBLinks(insertedNode.parent)
            ...
            if (node.key != 'null'){
                // console.log("we are checking the node: " + node.key);
                // console.log(node.left.isRed);
                // console.log(node.right.isRed);
                if (!node.left.isRed && node.right.isRed)    { nodeWasFine = false; rotateLeft(node); console.log('rotated left');  };
                if (node.left.isRed && node.left.left.isRed) { nodeWasFine = false; rotateRight(node); console.log('rotated right'); };
                if (node.left.isRed && node.right.isRed)     { nodeWasFine = false; flipColors(node); console.log('flipped colors'); };
        ...
        function rotateRight(node) {
            ...
            if (node.parent == null) {
                ...
                node.parent.left = node.left; // 975.left = 674
            } else {
                node.parent.right = node.left; // when node is on the right of its parent
            }
            node.left.parent = node.parent;   // 674.parent = 975.parent
            node.left.right = node; // 674.right = 975
            node.parent = node.left; // 975.parent = 674     
            node.left = nx; // 975.left = 674.right
            nx.parent = node; // 674.right.parent = 975
            ...
            node.left.distanceFromParent = -tree.w;
            node.right.distanceFromParent = tree.w;
            node.left.posX = node.left.posX - tree.w;
            node.right.posX = node.left.posX + tree.w;
            tree.updateDistances(node,node.right.posX);
        ...
        function rotateLeft(node) {
            // node = 559
            var nx = node.right.left; // nx is 674.left
            if (node.parent == null) {
                node.right.parent = null;
                tree.root = node.right;
                tree.root.dpth = 0;
            } else if (node == node.parent.left) {
                node.parent.left = node.right; // 975.left = 674
            } else {
                node.parent.right = node.right; // when node is on the right of its parent
            }
            node.right.parent = node.parent; // 674.parent = 975
            node.right.left = node;   // 674.left = 559
            node.parent = node.right; // 559.parent = 674
            node.right = nx; // 559.right = 674.left
            nx.parent = node; // 674.left.parent = 559
        ...
        function flipColors(node) {
            ...
            console.log("we are in flipColors!!!");
            node.isRed = true;
            node.left.isRed = false;
    ...
    update() {
        // if (background.tilePositionX != player.x) {
        //     background.tilePositionX = player.x;
        // }

        // if (background.tilePositionY != player.y) {
