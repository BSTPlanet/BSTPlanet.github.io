...
var  manualElement;
const node_472 = {x:935, y:813, name: "node_472"}

export class RotateLeft extends Phaser.Scene {

    constructor() {
        super({ key:'RotateLeft' });
    }
    ...
    create() {
        ...
        function makeRBTree(numsArray,scene) {
            ...
            tree.createTree(numsToInsert.slice(0,numsToInsert.length-1),scene);
            insertManualNode(scene)
            tree.openCurtains();
        ...
        function insertManualNode(scene) {
            // create the last node we insert manually 
            manualElement = numsToInsert[numsToInsert.length-1]
            tree.insertManual(tree.root, manualElement, scene)
            flipColorsManual(tree.root, manualElement, scene)
        }


        function flipColorsManual (node, key, scene) {
            if (node.key == key) {
                tree.flipColors(scene,node.parent)
                     
            } else if (node.key > key) {
                flipColorsManual(node.left, key, scene)

            } else if (node.key < key) {
                flipColorsManual(node.right, key, scene)
            }
        }
       ...
       // *************** TASKS + TASK ACTIONS ***************
        ...
        function taskSucceededActions() {
            ...
            if (tasks[0] == 'Rotate Left Again' && !(singleTon.nodeSetRB.has(node_472))) {
                singleTon.addNodeRB(node_472)
        ...
        // *************** CHECK ROTATE LEFT + ROTATE LEFT  + ANIMATIONS ***************
