...
const node_65 = {x:1155, y:813, name: "node_65"}

export class InsertBetweenNodes extends Phaser.Scene {

    constructor() {
        super({ key:'InsertBetweenNodes' });
    }
    ...
    create() {
       ...
       // *************** TASKS + TASK ACTIONS ***************
        ...
        function taskSucceededActions(scene) {
            ...
            if (tasks[0] == 65 && !(singleTon.nodeSetRB.has(node_65))) {
                console.log(singleTon.nodeSetRB)
                singleTon.addNodeRB(node_65)
            }
                ...
                this.time.addEvent({
                    ...
                    callback: function(scene) {
                        if (tasks[0] == data.task[0]) {  // if it's the first task then talk about that
                            expert.talk('insertBetweenNodes',4);
                        } else {                         // if it's the last task then talk about the last thing and give reward node
                            expert.talk('insertBetweenNodes',6);
                            panel.rewardNodeActions(65);
                        }
